...
class Pagination
{
	...
	public $_nItemOnPage; // số lượng item trong 1 trang
	private $_nPageShow ; // số lượng link page hiện thị
	...
	public function showPagination(){
		...
		if($this->_totalPage > 1){
			...
			if($this->_currentPage > 1){
				...
				$prev 	= "<li><a href='$actual_link&page=".($this->_currentPage-1)."'><i class='fa fa-caret-left'></i></a></li>";
			}
			$next 	= '';
			$end 	= '';
			if($this->_currentPage < $this->_totalPage){
				$next 	= "<li><a href='$actual_link&page=".($this->_currentPage+1)."'><i class='fa fa-caret-right'></i></a></li>";
			...
			for($i = $startPage; $i <= $endPage; $i++){
				if($i == $this->_currentPage) {
					$listPages .= "<li class='active'><span>".$i.'</span>';
				}else{
